package rpcnode.toolkit.agent.infrastructure.log
...
/** File log next to STDOUT. Call before the first application logger. */
fun installAgentFileLog(file: Path)
{
...
fun applyDevLogLevels(enabled: Boolean)
{
    ...
    (ctx.getLogger("rpcnode-agent") as Logger).level = Level.DEBUG
}
