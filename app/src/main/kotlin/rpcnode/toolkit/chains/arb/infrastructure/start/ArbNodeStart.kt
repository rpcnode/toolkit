...
import rpcnode.toolkit.chains.arb.infrastructure.arbL1FromInstallOptions
import rpcnode.toolkit.chains.arb.infrastructure.arbSnapshotFlavor
...
/**
 * Arbitrum Nitro full node. Launch args encode env + disk roles + snapshot flavor
 * for [rpcnode.toolkit.chains.arb.infrastructure.proc.ArbNodeProcessStarter].
 */
class ArbNodeStart : ChainNodeStart
{
    override val networkId: NetworkId = NetworkId.ARB

    override fun plan(ctx: ChainNodeStartContext): ChainNodeStartPlan
    {
        val env = ctx.env.trim().lowercase().ifEmpty { "mainnet" }
        val cluster = ArbClusters.lookup(env)
        val flavor = arbSnapshotFlavor(ctx.installOptionsJson)
        val l1 = arbL1FromInstallOptions(ctx.installOptionsJson, cluster.env)
        ...
        return ChainNodeStartPlan(
            launch = NodeLaunchSpec(
                ...
                entry = "nitro",
                args = args,
                extractArchiveGlob = "nitro-*.tar.gz",
                normalizeDir = null,
            ),
