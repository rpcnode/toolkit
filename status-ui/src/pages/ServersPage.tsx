...
export function ServersPage() {
  ...
  async function saveEdit() {
    ...
    try {
      const res = await api.registryUpsert({
        ...
        network: editTarget.network || 'tron',
  ...
  async function confirmRemove() {
    ...
    try {
      ...
      notifications.show({ color: 'teal', message: `Removed server ${removeTarget.name || removeTarget.id}` })
      setRemoveTarget(null)
      void load()
    } catch (e) {
      notifications.show({ color: 'red', message: String((e as Error).message || e) })
  ...
  return (
    ...
    >
      <Stack gap="md" mt="md">
        {error && (
          <Alert color="red" icon={<IconAlertTriangle size={16} />} title="Servers error">
            {error}
          </Alert>
        )}
        ...
        {items.length === 0 ? (
          ...
          <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="md">
            {items.map((s) => {
              ...
              const m = s.metrics
              const st = s.metrics_status || 'unknown'
              ...
              return (
                <Card key={s.id} className="env-card server-card">
                  <Group
                  ...
                  <Stack gap={8} style={{ flex: 1 }} className="server-card__body">
                    ...
                    <SimpleGrid cols={2} spacing="xs" verticalSpacing={8} className="server-card__metrics">
                      ...
                      <MetricCell
                        ...
                        hint={hasMetrics(m) ? 'Root filesystem used / total' : 'No metrics yet'}
                      />
                      <MetricCell
                        label="Load avg"
                      ...
                      >
                        <span>
                          <ActionIcon
                            ...
                            disabled={(!needsUpdate && !updating) || (!!updatingId && !updating)}
                  ...
                  <Group className="env-card__actions" gap="xs" mt="sm" wrap="nowrap">
                    ...
                    <Tooltip label="Agent logs (tip / leaf / watchdog)">
                      <ActionIcon
                        size="md"
                        variant="light"
                        color="gray"
                        aria-label="Server agent logs"
          ...
          {(removeTarget?.nodes_count ?? 0) > 0 ? (
            ...
            <>
              <Text size="sm">
                ...
                from the panel registry? This does not stop agents on the host.
              </Text>
              <Text size="sm" c="dimmed">
                No nodes attached — safe to remove from the panel.
              </Text>
              <Alert color="gray" title="Clean agents on the host (optional)">
                <Stack gap="xs">
                  <Text size="sm">
                    Tip + leaf agents + watchdog only. Fullnodes and datadirs stay.
                  </Text>
                  <Code block className="mono" style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
                    {UNINSTALL_AGENTS_CMD}
                  </Code>
                  <Button
                    size="xs"
                    variant="light"
                    ...
                    onClick={() => {
                      void copyText(UNINSTALL_AGENTS_CMD)
                        .then(() => {
                          notifications.show({ color: 'blue', message: 'Uninstall command copied' })
                        })
                        .catch(() => {
      ...
      >
        <Stack gap="md">
          <Alert color="yellow" icon={<IconAlertTriangle size={16} />}>
            Downloads <strong>api-agent + system-agent</strong> from CDN and restarts their systemd
            units. Brief disconnect possible. UI waits {AGENT_UPDATE_SETTLE_SEC}s, then checks the
            new version.
          </Alert>
          <Text size="sm">
            <Text span fw={700}>
      ...
      >
        <Stack gap="md">
          ...
          <TextInput
            label="Agent URL"
            description="Host agent control URL (after provision this is often :39091, not :39090)"
            className="mono"
            value={editURL}
            onChange={(e) => setEditURL(e.currentTarget.value)}
            placeholder="http://203.0.113.10:39091"
          />
          <PasswordInput
