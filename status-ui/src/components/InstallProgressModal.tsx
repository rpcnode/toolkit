...
export function InstallProgressModal({
  ...
  const [streamId, setStreamId] = useState('host')
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const streams = (res.streams || []).filter((s) => streamRelevantToNode(s, network, env))
      setPayload({ ...res, streams })
      setFetchedAt(new Date())
      setStreamId((prev) => {
        if (prev && streams.some((s) => s.id === prev)) return prev
        if (streams.some((s) => s.id === 'host')) return 'host'
        return streams[0]?.id || ''
      })
    } catch (e) {
  ...
  return (
    <Modal
      ...
      centered
      size="xl"
      closeOnClickOutside={outcome !== 'running'}
      ...
      <Stack gap="sm">
        ...
        <Group justify="space-between" wrap="wrap">
          <Text size="xs" c="dimmed">
            {active?.path || '/var/log/rpcnode.log'}
