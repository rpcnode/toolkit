...
type Props = {
  ...
  serverId: string
  serverName?: string
  /** Prefer this stream id when the modal opens (e.g. host audit). */
  defaultStream?: string
  /** When set, hide other networks' leaf streams (keep host + tip + this node). */
  network?: string
  env?: string
}

function streamRelevantToNode(s: AgentLogStream, network?: string, env?: string): boolean {
  ...
  const slug = `${network}-${env || ''}`.toLowerCase()
...
export function ServerLogsModal({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const streams = (res.streams || []).filter((s) =>
        streamRelevantToNode(s, network, env),
      )
      setPayload({ ...res, streams })
      setStreamId((prev) => {
        if (prev && streams.some((s) => s.id === prev)) return prev
        if (defaultStream && streams.some((s) => s.id === defaultStream)) {
  ...
  const title = `Agent logs · ${serverName || serverId}`
      ...
      <Stack gap="sm">
        <Group justify="space-between" wrap="wrap">
          <Text size="xs" c="dimmed">
            Host audit + tip / leaf / watchdog + TRON node · v
            {payload?.version || '—'}
          </Text>
          <Group gap={6}>
        ...
        {loading && !payload ? (
          ...
          <Text size="sm" c="dimmed">
            No agent units found on this host.
          </Text>
        ) : (
          <>
            <SegmentedControl
