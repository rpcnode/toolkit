...
func (s *Server) handleAuthStatus(w http.ResponseWriter, r *http.Request) {
	...
	user := ""
	authed := false
	// SPA gate is session-cookie only. Do not treat HTTP Basic as "logged in" —
	// browsers cache Basic and would bounce /login → dashboard after logout.
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"user":               user,
		"session_ttl_days":   int(sessionTTL / (24 * time.Hour)),
		"agent_download_url": defaultAgentDownloadURL(),
		"links": map[string]string{
			"rpcnode": "https://rpcnode.dev",
		},
		"auth_modes": []string{"session_cookie", "session_bearer", "panel_basic_api_tools"},
		"note":       "SPA uses session cookie. API tools: Authorization Bearer <token> from POST /api/auth/login (or ./scripts/panel-token.sh). HTTP Basic against htpasswd also works for non-browser clients.",
	})
}
