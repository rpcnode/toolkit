...
function syncProgressFromDb(height?: number | null, tip?: number | null): number | undefined {
  if (height == null || tip == null || !Number.isFinite(Number(height)) || !Number.isFinite(Number(tip))) {
    return undefined
  }
  const h = Number(height)
  const t = Number(tip)
  if (t <= 0 || h < 0) return undefined
  return Math.min(100, Math.max(0, (h / t) * 100))
}
...
function lifecycleFromDB(w: Workload): NodeLifecycle {
  ...
  const height = w.height == null ? null : Number(w.height)
  const tip =
    w.network_height == null || Number(w.network_height) <= 0 ? null : Number(w.network_height)
  const dbProgress = syncProgressFromDb(height, tip) ?? w.snapshot_progress ?? undefined
  if (wl === 'removing' || lifePhase === 'removing' || lifeLabel === 'removing') {
    return {
      phase: 'removing',
      label: 'removing',
      detail: w.lifecycle_detail || 'Wiping host folders — row drops when they are gone',
      color: 'orange',
      busy: true,
      height,
    }
  ...
  // Kotlin panel statuses — drive sync bar + height from SQLite.
  if (wl === 'sync') {
    return {
      phase: 'syncing',
      ...
      detail:
        ...
        (height != null
          ? tip != null
            ? `Height ${height.toLocaleString()} · tip ${tip.toLocaleString()}`
  ...
  if (wl === 'active') {
    return {
      phase: 'working',
      label: w.lifecycle_label || 'Synced',
      detail:
        w.lifecycle_detail ||
        (height != null ? `Height ${height.toLocaleString()} · online` : 'Node online'),
      color: 'teal',
      busy: false,
  ...
  if (wl === 'stopped') {
    return {
      phase: 'unknown',
      label: w.lifecycle_label || 'stopped',
      detail: w.lifecycle_detail || 'Node stopped',
      color: 'gray',
      busy: false,
      height,
    }
  }
  const noSnap = w.needs_snapshot !== true && isNoSnapshotNetwork(w.network)
  ...
  if (w.lifecycle_phase) {
    ...
    const shortHealthy =
      /^(healthy|running|working)$/i.test(label.trim()) && (height ?? 0) > 0
  ...
  const fallback = deriveNodeLifecycle(null, w.status, w.network, w.needs_snapshot)
  return { ...fallback, height: fallback.height ?? height, progress: fallback.progress ?? dbProgress }
}
...
function networkRank(network: string): number {
  ...
  const idx = getNetworks().findIndex((n) => n.id === id)
...
export function NodesPage() {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (addOpen || !hasClients || !isModN(e) || isTypingTarget(e.target)) return
      e.preventDefault()
      setAddOpen(true)
    }
  ...
  const load = useCallback(async (opts?: { silent?: boolean; refresh?: boolean }) => {
    ...
    try {
      const [workloads, registry, clients] = await Promise.all([
      ...
      const next: NodeCard[] = items.map((w) => {
        ...
        return {
          ...
          liveTestStatus: w.live_test_status || '',
          liveTestAt: w.live_test_at || '',
          liveTestError: w.live_test_error || '',
          needsSnapshot: w.needs_snapshot === true,
          installed: true,
  ...
  const networkGroups = useMemo(() => {
    ...
    networks.forEach((n, i) => order.set(n.id, i))
  ...
  return (
    ...
    >
      <div className={`nodes-page-layout${cards.length > 0 ? ' nodes-page-layout--with-nav' : ''}`} {...blockProps('nodes.layout')}>
        {cards.length > 0 && (
          <aside className="nodes-net-nav" aria-label={groupBy === 'network' ? 'Networks on this page' : 'Servers on this page'} {...blockProps('nodes.nav')}>
            <div className="nodes-net-nav__toggle" role="tablist" aria-label="Group nodes by">
              <UnstyledButton
                type="button"
                role="tab"
                aria-selected={groupBy === 'network'}
                className={`nodes-net-nav__toggle-btn${groupBy === 'network' ? ' is-active' : ''}`}
                onClick={() => applyGroupBy('network')}
              >
                <IconTopologyStar3 size={14} stroke={1.6} aria-hidden />
                networks
              </UnstyledButton>
              <UnstyledButton
                type="button"
                role="tab"
                aria-selected={groupBy === 'server'}
                className={`nodes-net-nav__toggle-btn${groupBy === 'server' ? ' is-active' : ''}`}
                onClick={() => applyGroupBy('server')}
              >
                <IconServer size={14} stroke={1.6} aria-hidden />
                servers
              </UnstyledButton>
            </div>
            <ScrollArea
            ...
            >
              <ol className="panel-rail nodes-net-nav__list">
                <li>
                  <UnstyledButton
                    type="button"
                    className={`panel-rail__item nodes-net-nav__item${!activeNavKey ? ' is-active' : ''}`}
                    onClick={() => jumpToGroup(null)}
                  >
                    <span className="nodes-net-nav__icon" aria-hidden>
                      {groupBy === 'network' ? (
                    ...
                    <span className="nodes-net-nav__text">
                      <span className="panel-rail__label">
                        {groupBy === 'network' ? 'all networks' : 'all servers'}
                      </span>
                    </span>
                    <span className="nodes-net-nav__count">{cards.length}</span>
                  </UnstyledButton>
                </li>
                {railNav.map((n) => (
                  <li key={n.key}>
                    <UnstyledButton
                      ...
                      className={`panel-rail__item nodes-net-nav__item${activeNavKey === n.key ? ' is-active' : ''}`}
                      onClick={() => jumpToGroup(n.key)}
                    >
                      <span className="nodes-net-nav__icon" aria-hidden>
                        {groupBy === 'network' ? (
                          <NetworkIcon network={n.key} size={16} />
                        ) : (
                          <IconServer size={15} stroke={1.5} />
                        )}
                      </span>
                      <span className="nodes-net-nav__text">
                        <span className="panel-rail__label">{n.label}</span>
                        {n.sub ? (
                          <span className="nodes-net-nav__sub">{n.sub}</span>
                        ) : null}
                      </span>
                      <span className="nodes-net-nav__count">{n.count}</span>
                    </UnstyledButton>
                  </li>
                ))}
              </ol>
        ...
        <Stack gap="md" className="nodes-page-content" {...blockProps('nodes.content')}>
          {error && (
          ...
          {cards.length === 0 ? (
              ...
              <Stack gap="sm">
                ...
                <Group>
                  {hasClients ? (
                    ...
                    <Button color="teal" onClick={() => navigate({ name: 'clients' })}>
                      Go to Clients
                    </Button>
                  )}
            ...
            <Card {...blockProps('nodes.filter-empty')}>
              <Stack gap="sm">
            ...
            <Stack gap="xl" {...blockAttrs('nodes.list', { domId: 'nodes-list-top' })}>
              {flatNetworks ? (
                <SimpleGrid cols={{ base: 1, sm: 2, lg: 2, xl: 3 }} spacing="md">
                  {cardsByNetworkLabel.map((c) => (
                    <NodeCardView
                      ...
                      onLiveTest={(nid, result) => {
                        setCards((prev) =>
                          prev.map((x) =>
                            x.id === nid
                              ? {
                                  ...x,
                                  liveTestStatus: result.status,
                                  liveTestAt: result.at,
                                  liveTestError: result.error,
                                }
                              : x,
                          ),
                        )
...
function NodeCardView({
  ...
  onLiveTest?: (
    id: string,
    result: { status: 'pass' | 'fail'; at: string; error: string },
  ) => void
  anchorId?: string
}) {
  const {
  ...
  function openLogs(e: { stopPropagation: () => void }) {
    e.stopPropagation()
    setLogsOpen(true)
  }
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const res = await api.clientInfo(id)
  ...
  async function confirmClientRollback() {
    ...
    try {
      const res = await api.clientUpdateRollback(id)
  ...
  async function confirmNodeStop() {
    setStopBusy(true)
    try {
      const res = await api.workloadsNodeProcessStop(id)
  ...
  async function confirmNodeRestart() {
    setRestartBusy(true)
    try {
      const res = await api.workloadsNodeProcessStart(id)
  ...
  async function runLiveTest(e: { stopPropagation: () => void }) {
    e.stopPropagation()
    setTestBusy(true)
    try {
      const res = await api.workloadsTest({ node_id: id })
      ...
      onLiveTest?.(id, {
        status: st === 'pass' ? 'pass' : 'fail',
        at: res.live_test_at || new Date().toISOString(),
        error: res.live_test_error || res.error || '',
      })
    } catch (err) {
      notifications.show({ color: 'red', message: String((err as Error).message || err) })
    } finally {
  ...
  return (
    <div
      className={anchorId ? 'node-card-cell nodes-network-section' : 'node-card-cell'}
      id={anchorId}
      {...blockData(`nodes.card.${id}`)}
    >
    <Card
      className={[
        'env-card',
        'env-card--installed',
        liveTestStatus === 'fail' ? 'env-card--live-fail' : '',
        liveTestStatus === 'pass' ? 'env-card--live-pass' : '',
      ]
        .filter(Boolean)
        .join(' ')}
    ...
    >
      <Group justify="space-between" mb="sm" align="flex-start" style={{ flexShrink: 0 }}>
        <Group gap="sm">
          <NodeStatusIcon phase={phase} busy={busy && !working} />
          <div>
        ...
        <Group gap={6} wrap="nowrap" style={{ flexShrink: 0 }}>
          ...
          {nodeLiveTestAllowed(
            ...
            <Tooltip
              label={
                liveTestStatus === 'fail'
                  ? model.liveTestError || 'Tests failed'
                  : liveTestStatus === 'pass'
                    ? 'Tests passed'
                    : 'Test'
              }
            >
              <ActionIcon
                ...
                variant="subtle"
                color={
                  liveTestStatus === 'fail' ? 'red' : liveTestStatus === 'pass' ? 'teal' : 'gray'
                }
                aria-label="Test"
                loading={testBusy}
                disabled={isRemoving || testBusy}
                onClick={(e) => void runLiveTest(e)}
              >
                <IconFlask size={14} />
              </ActionIcon>
            </Tooltip>
        ...
        {isRemoving || isRemoveError ? (
          <Alert color="orange" variant="light" p="xs" icon={<IconTrash size={14} />}>
            ...
            <Text size="xs" c="dimmed">
              {detail ||
                (isRemoveError
                  ? 'Wipe did not finish — open Remove and Retry (tip resumes kill → units → wipe)'
                  : 'Wiping host folders — row stays until they are gone')}
            </Text>
          </Alert>
        ) : phase === 'error' ? (
        ...
        <div className="node-card__client">
          ...
          <Group gap={6} align="center" wrap="nowrap">
            ...
            >
              ...
              >
                {clientLabel}
                {clientOutdated && clientLatestLabel ? ` → ${clientLatestLabel}` : ''}
              </Text>
            </Tooltip>
          </Group>
          {phase === 'updating' ? (
