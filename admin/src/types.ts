...
export type InstanceInfo = {
  ...
  registered?: boolean
  /** RPC public port (RPCNODE_PUBLIC_PORT). */
  gateway_port?: number
  public_port?: number
  /** Ops panel port (RPCNODE_PANEL_PORT). */
  panel_port?: number
...
/** Classified agent ERROR lines from /var/log/rpcnode/errors/<network>-<env>.log */
export type AgentErrorsInfo = {
  path?: string
  network?: string
  env?: string
  lines?: string[]
...
/** Per-network install walk: /var/log/rpcnode/<network>-install.log */
export type InstallLogInfo = {
  path?: string
  network?: string
  env?: string
  lines?: string[]
...
export type LifecycleStep = {
  ...
  active?: boolean
  error?: boolean
  /** True when this step failed and the wizard may retry it (keep snapshot). */
  retryable?: boolean
...
export type LifecycleProfile = {
  ...
  /** ExtraStep is the chain unit (Solana / Robinhood) — not a tgz oneshot. */
  snapshot_via_node?: boolean
  snapshot_bootstrap?: string
...
export type LifecycleInfo = {
  ...
  current?: string
  /** First lane step with status=error — UI highlights + Retry this step only. */
  failed_step?: string
  ...
  /** Host path /var/log/rpcnode/<network>-install.log */
  install_log?: string
}
...
export type StatusPayload = {
  ...
  message?: string
  error?: string
  /** Agent-owned start fault (Agave down after snapshot, missing conf, …). */
  start_error?: string
  public_base?: string
  ...
  /** Build of that agent version: `<sha>[-dirty] <HH:MM>`. */
  agent_build?: string
  ...
  logs?: AgentLogsInfo
  /** Classified agent errors (hostLog ERROR / start_error / snapshot_error). */
  errors?: AgentErrorsInfo
  /** Per-network install steps file + tail. */
  install_log?: InstallLogInfo
  ...
  connect?: {
    ...
    note?: string
    /** Additional named upstream reachable through the same public port
     *  (e.g. TRON SolidityNode at /walletsolidity). Catalog-driven, empty
     *  for networks with a single upstream. */
    endpoints?: Array<{ id: string; label: string; url: string }>
  }
  panel_base?: string
  node_status?: string
  ...
  /** On-disk path(s) of this node's own config file(s) — system-agent attachNodeConfigPath. */
  config?: {
    path?: string
    documents?: Array<{ id: string; path: string; title?: string }>
  }
}
