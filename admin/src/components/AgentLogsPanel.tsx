...
import { chainDataPath, chainEtcPath, chainOptPath } from '../lib/networkPaths'
...
export function agentLogPaths(status: StatusPayload | null | undefined): string[] {
  ...
  if (net === 'ton') {
    ...
    return [
      ...
      `${chainEtcPath('ton', env)}/sync-progress.log`,
      'journalctl -u validator.service',
      'journalctl -u ton_http_api.service',
    ]
  }
  if (net === 'solana') {
    paths.push(`${chainDataPath('solana', env)}/solana-${env}.log`)
  }
  if (net === 'tron') {
    return [
      `${chainOptPath('tron', env)}/logs/tron.log`,
...
export function AgentLogsPanel({ status }: { status: StatusPayload }) {
  ...
  return (
    <Card {...blockProps('node.detail.agent-logs')}>
...
export function AgentLogsModal({
  ...
  return (
    <Modal opened={opened} onClose={onClose} title={title} centered size="xl" {...blockProps('modal.agent-logs')}>
