...
import { chartPairSeries, chartNetSeries, chartSeries, formatStorageGiB, fmtDiskFree, fmtDiskMBs, fmtIOPS, fmtMbps, num } from '../lib/format'
...
export function MetricCharts({
  ...
  forceRpcPanel = false,
  /** Hide Fullnode Go RPC (e.g. during setup wizard). */
  showRpcPanel = true,
  ...
  const hasHostNet = cur?.net_rx_mbps != null || cur?.net_tx_mbps != null || hostNet.length > 0
  const hasHistoryCharts = cpu.length > 0 || mem.length > 0 || load.length > 0
  const hasCapacityDisks = (cur?.disks?.length || 0) > 0

  const hostCard = (
    <Card {...blockProps('node.detail.metrics.host')}>
      <Group justify="space-between" mb="md" wrap="wrap" gap="sm">
        <div>
          <Title order={3}>Host</Title>
          <Text c="dimmed" size="xs">
            Host CPU / RAM / NIC / disk from agent heartbeat (panel cache)
            {metrics?.updated_at ? ` · ${metrics.updated_at}` : ''}
          </Text>
        </div>
        <Group gap="md" wrap="wrap">
          <Stat
            ...
            hint="Host CPU busy percent for this server (/proc/stat)."
          />
          <Stat
            label="Host Mem"
          ...
          />
          {cur?.load_1 != null && !Number.isNaN(cur.load_1) ? (
            <Stat
              label="Load 1m"
              value={num(cur.load_1, 2)}
              hint="1-minute system load average (Linux loadavg)."
            />
          ) : null}
          <Stat
            label="Host Net ↓"
  ...
  return (
    <Stack gap="md" {...blockProps('node.detail.metrics-charts')}>
      {/* Sync first — then Host (full width) — then Fullnode Go RPC. */}
      {sidePanel}
      {hostCard}

      {showRpc ? (
      <Card className="fullnode-rpc-card" {...blockProps('node.detail.metrics.rpc')}>
        <Group justify="space-between" mb="md" wrap="wrap" gap="sm">
          <div>
    ...
    <Card padding="sm" className="metric-chart-panel" mb="md">
      {items.length > 1 ? (
        <Tabs value={active} onChange={setTab} mb="sm">
          <Tabs.List className="disk-tabs-list" style={{ flexWrap: 'nowrap' }}>
            <Tabs.Tab value="all">
              All
