...
import { formatSyncPct, num, parseAria2PctFromText, parseSolanaDownloadPctFromText } from '../lib/format'
...
function parseAria2PctFromStatus(status?: StatusPayload | null): number | null {
  if (!status) return null
  const chunks: string[] = []
  if (status.snapshot?.detail) chunks.push(String(status.snapshot.detail))
  if (status.lifecycle?.detail) chunks.push(String(status.lifecycle.detail))
  if (status.sync?.detail) chunks.push(String(status.sync.detail))
  const tails = status.sync?.log_tail || status.logs?.lines
  if (Array.isArray(tails)) chunks.push(tails.join('\n'))
  return parseAria2PctFromText(chunks.join('\n'))
}

function parseSolanaDownloadPctFromStatus(status?: StatusPayload | null): number | null {
  if (!status || !isSolanaNetwork(resolveNetwork(status))) return null
  const chunks: string[] = []
  if (status.snapshot?.detail) chunks.push(String(status.snapshot.detail))
  if (status.lifecycle?.detail) chunks.push(String(status.lifecycle.detail))
  if (status.sync?.detail) chunks.push(String(status.sync.detail))
  const tails = status.sync?.log_tail || status.logs?.lines
  if (Array.isArray(tails)) chunks.push(tails.join('\n'))
  return parseSolanaDownloadPctFromText(chunks.join('\n'))
}
...
export function resolveSyncProgressPct(status?: StatusPayload | null): number | null {
  ...
  // Snapshot download (TRON aria2 ExtraStep) — real % from agent logs.
  // Solana Agave download % is Sync (via-node), not Snapshot ExtraStep.
  const ns = (status?.node_status || status?.lifecycle?.node_status || '').toLowerCase()
  const pastSnapshot =
    ns === 'sync' || ns === 'syncing' || ns === 'starting' || ns === 'active' || ns === 'online'
  const snapPct =
    asPct100(status?.snapshot?.pct ?? status?.snapshot?.progress_pct) ??
    parseAria2PctFromStatus(status) ??
    (pastSnapshot || isSolanaNetwork(resolveNetwork(status))
      ? null
      : parseSolanaDownloadPctFromStatus(status))
  ...
  const ariaLive = snapPct != null && snapPct > 0 && snapPct < 99.9
  const snapBusy =
    !snapAborted &&
    !pastSnapshot &&
...
export function showSyncStatusCard(
  ...
  if (
    ...
    phase === 'snapshot' ||
    ns.includes('snapshot') ||
    parseAria2PctFromStatus(status) != null ||
    isBscNetwork(net)
  ) {
    return true
  }
...
export function SyncStatusCard({
  ...
  const detail =
    ...
    (regtest
      ...
      : snapBusy
        ? status.snapshot?.detail || 'Official snapshot'
        : syncing
          ? 'Syncing'
          : 'Sync status')
  const blocks = sync?.blocks ?? sync?.block ?? sync?.height
  // Agent/DB verification_pct only — never invent a UI floor (0.1% stays 0.1%).
  // Snapshot ExtraStep with no aria2 sample yet: animated stub bar, label "…" not "0%".
  const snapWaiting = snapBusy && (progress == null || progress <= 0)
  const barValue = snapWaiting ? 6 : progress != null ? progress : 0
  const pctLabel = snapWaiting ? null : progress
  ...
  return (
    <Card padding="md" {...blockProps('node.detail.sync-card')}>
      ...
      {hasServer ? (
        <Group gap={6} wrap="wrap" mb={hasSync ? 8 : 0} align="center">
      ...
      {hasSync ? (
        <>
          <Group align="flex-end" gap="sm" mb={4} wrap="wrap">
          ...
          <Progress
            value={barValue}
            color={snapBusy || syncing ? 'yellow' : sync?.ok === false ? 'red' : 'teal'}
            size="sm"
            radius="xl"
            mb={8}
            animated={snapBusy || syncing || (progress != null && progress < 100)}
            striped={snapBusy || syncing || (progress != null && progress < 100)}
          />
          )}
