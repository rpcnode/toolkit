...
export function snapshotUIMode(
  ...
  if (!supportsSnapshotStep(status, network, envHint ?? resolveEnv(status))) return 'hidden'
...
function SnapshotLogModal({
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Snapshot log" size="lg" centered {...blockProps('modal.snapshot-log')}>
