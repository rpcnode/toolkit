...
```bash
curl -fsSL -o rpcnode-agent.jar "$PANEL/install/binaries/rpcnode-agent.jar"
...
Panel proxy (ops SPA): `GET /api/host/disks?server_id=` → agent `GET /api/v1/host/disks` (inventory only).

Panel proxy (ops SPA): `GET /api/host/sysctl?server_id=` → agent `GET /api/v1/host/sysctl` — live `/proc/sys` for Agave knobs plus Anza recommended floors and `install_option_keys` (Start Client config).

Panel disk layout (SQLite, per node UUID):

- `GET /api/nodes/{uuid}/disk-layout` → `{ ok, node_id, network, env, disk_layout, multi_disk_roles, layout_rules, recommended?, install_options }`
- `PUT /api/nodes/{uuid}/disk-layout` body `{ "disk_layout": { … } }`
- Also on `GET /api/nodes/{uuid}` / list items as `disk_layout` and `install_options`
- `disk_layout.roles[]` — per-network storage plan (Solana: ledger/accounts/snapshots; TRON: fullnode/solidity; Bitcoin: blockchain/index). Catalog from `chains/<id>/network.yml` `diskRoles`.
- `multi_disk_roles` — same catalog definitions with labels and `size_hint_gib` for the node's env.
- `recommended` — JBOD suggestion from host agent when reachable; omitted when agent is down.
...
Preferred: panel **Servers → Update agent** (or `POST /api/v1/agent/update`) — the agent downloads `rpcnode-agent.jar` from the panel `/install/binaries/` and restarts. Chain client / datadir are not touched.
